...
int main(int argc, char* argv[])
{
    ...
    while (running)
    {
        ...
        {
            // Timed out
            if (termkey_getkey_force(tk, &key) == TERMKEY_RES_KEY) on_key(tk, &key);
        }

        if (fd.revents & (POLLIN | POLLHUP | POLLERR)) termkey_advisereadable(tk);
        ...
        if (ret == TERMKEY_RES_AGAIN) nextwait = termkey_get_waittime(tk);
        else nextwait = -1;
    }
